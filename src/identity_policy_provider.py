...
request_schema = {
    ...
    "properties": {
        "Identity": {"type": "string", "description": "that the policy will apply to"},
        "PolicyName": {
            "type": "string",
            "maxLength": 64,
            "pattern": "^[A-Za-z-_]+$",
        ...
        "PolicyDocument": {
            ...
            "properties": {
                ...
                "Version": {"type": "string", "description": "of the policy document"},
            },
        },
    },
}
...
class IdentityPolicyProvider(ResourceProvider):
    ...
    def update(self):
        if (
            self.policy_name != self.old_policy_name
    ...
    def delete(self):
        ...
        if self.physical_resource_id != "could-not-create":
            try:
                self.ses.delete_identity_policy(
                    Identity=self.identity, PolicyName=self.policy_name
                )
            except ClientError as e:
    ...
    def get_policy(self, identity, policy_name):
        try:
            current_policies = self.ses.get_identity_policies(
                Identity=identity, PolicyNames=[policy_name]
            )["Policies"]
            return current_policies.get(policy_name)
        except ClientError as e:
            self.fail(f"failed to retrieve identity policy {policy_name}, {e}")
