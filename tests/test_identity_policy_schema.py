...
def test_list_of_services():
    identity = f"{uuid.uuid4()}.internal"
    request = Request(
        "Create", identity, {"Service": ["ecs.amazonaws.com", "eks.amazonaws.com"]}
    )
    provider.set_request(request, {})
    assert provider.is_valid_request(), provider.reason
...
def test_canonical_user():
    identity = f"{uuid.uuid4()}.internal"
    request = Request(
        "Create", identity, {"CanonicalUser": "dfjsdfskdfsfdhjsdfkjsdfjk"}
    )
    provider.set_request(request, {})
    assert provider.is_valid_request(), provider.reason
...
class Request(dict):
    def __init__(self, request_type, identity, principals):
        ...
        self["ResourceProperties"]["PolicyDocument"]["Statement"][0][
            "Principal"
        ] = principals
